...
int main() {
	...
	// modify the rules as stated in the exercise
	rules[8].new_subrule();
	rules[8].add_subrule(42);
	rules[8].add_subrule(8);

	rules[11].new_subrule();
	rules[11].add_subrule(42);
	rules[11].add_subrule(11);
	rules[11].add_subrule(31);

	int matches = 0;
	string line;

	while (getline(cin, line)) {
		bool matched = match(rules, line);
		matches += matched;
	}
