...
int main() {
	...
	unordered_map<string, vector<string>> allergen_map;
	...
	// remove allergens from the ingredient lists
	for (auto &am : allergen_map) {
		string allergen = am.second.front();

		for (auto &food : dataset) {
			food.ingredients.erase(remove(begin(food.ingredients), end(food.ingredients), allergen), end(food.ingredients));
		}
	}

	cout << accumulate(begin(dataset), end(dataset), 0, [](int a, auto &food) {return a + food.ingredients.size();}) << endl;
